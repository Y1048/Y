    ...
    for body in robot_body.iter("body"):
        ...
        for geom in body.findall("geom"):
            ...
            if not geom.get("name"):
                geom.set(
                    "name",
                    f"mink_collision_{body_name}_{local_index}_{name_counter}",
                )
            local_index += 1
            name_counter += 1
...
def _build_collision_pairs(model: mujoco.MjModel) -> tuple[list[tuple[list[str], list[str]]], list[tuple[int, int]]]:
    ...
    for index, (body1, geom1_name) in enumerate(records):
        for body2, geom2_name in records[index + 1 :]:
            first_is_arm = body1 in right_arm_body_ids
            second_is_arm = body2 in right_arm_body_ids
...
def _select_solver() -> str:
    ...
    for candidate in ("daqp", "proxqp", "quadprog", "osqp"):
        if candidate in available:
            return candidate
    raise RuntimeError(
        "No supported QP backend found. Install DAQP: py -3.11 -m pip install daqp"
    )
...
def _min_pair_distance(
    ...
    for geom1, geom2 in geom_pairs:
        distance = float(
            mujoco.mj_geomDistance(model, data, geom1, geom2, distmax, fromto)
        )
        if distance >= distmax:
            continue
...
def _send_unity_state(
    sock: socket.socket,
    configuration,
    right_qpos_ids: list[int],
    active: bool,
