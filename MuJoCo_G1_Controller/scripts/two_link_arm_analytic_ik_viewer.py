...
xml = """
<mujoco model="two_link_arm_analytic_ik">
...
with mujoco.viewer.launch_passive(model, data) as viewer:
    ...
    while viewer.is_running():
        ...
        target_pos = np.array([
            0.18 + 0.08 * math.sin(0.45 * t),
            0.0,
            0.72 + 0.06 * math.sin(0.7 * t),
        ])
        ...
        if t - last_print > 0.25:
            ...
            dist = np.linalg.norm(target_pos - wrist_pos)
            print(
                f"target=({target_pos[0]: .3f}, {target_pos[2]: .3f}) "
