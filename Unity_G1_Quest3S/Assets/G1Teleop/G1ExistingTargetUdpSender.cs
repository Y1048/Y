...
public class G1ExistingTargetUdpSender : MonoBehaviour
{
    ...
    // Keep teleop active unless the wrist stays outside workspace longer.
    ...
    private void Update()
    {
        if (!Application.isPlaying || shutdown_complete)
        {
            return;
        }

        if (hand_binder == null && right_hand_target == null)
        {
            return;
        }
    ...
    private void SendTarget()
    {
        ...
        if (calibration_started)
        {
            ...
            backend_workspace_rearm_revision = state_receiver == null
                ? 0
                : state_receiver.StateRevision;
        ...
        // Once engaged, temporary tracking loss holds the last target instead
        // of releasing the clutch. Only workspace exit resets calibration.
        ...
        Vector3 operator_delta = hand_binder == null
            ? Vector3.zero
        ...
        bool local_workspace_exit = command_valid
            && IsOutsideRobotWorkspace(UnclampedRobotTarget);
        bool backend_workspace_exit = command_valid
            && state_receiver != null
            && state_receiver.HasRecentState
            && state_receiver.IsWorkspaceLimited;
        bool backend_feedback_armed = !workspace_exit_latched
            && !backend_workspace_rearm_pending;

        bool workspace_exit_candidate = disengage_on_workspace_exit
            && ShouldDisengageForWorkspace(
                local_workspace_exit,
                backend_workspace_exit,
                backend_feedback_armed);
        workspace_exit_duration = UpdateWorkspaceExitDuration(
            workspace_exit_candidate,
            workspace_exit_duration,
            send_interval);
        bool workspace_exit_confirmed = IsWorkspaceExitConfirmed(
            workspace_exit_duration,
        ...
        if (workspace_exit_confirmed)
        {
            ...
            if (first_exit)
            {
                string exit_source = local_workspace_exit
                    ? "Unity workspace boundary"
                    : "MuJoCo workspace boundary";
                Debug.LogWarning(
                    "G1 teleoperation disengaged at " + exit_source
        ...
        else
        {
            if (workspace_exit_candidate && !workspace_violation_hold_active)
            {
                ...
                Debug.Log(
                    "G1 workspace excursion detected; continuing with workspace-clipped targets and "
                    + "will rebase on recovery."
                );
            }
            ...
            if (!workspace_exit_candidate && workspace_violation_hold_active)
            {
                workspace_violation_hold_active = false;
                Debug.Log(
                    "G1 workspace excursion cleared; re-centering hand-to-robot mapping."
                );
                ...
                if (hand_binder != null && hand_binder.IsTrackingValid && hand_binder.IsCalibrated)
                {
                    ...
                    target_rotation = hand_binder.MappedHandRotation;
                }
            }
        ...
        string json_text = BuildPacket(
            send_position,
            send_rotation,
            command_valid,
            command_state);
        if (!SendPacket(json_text))
        {
            return;
        }
        ...
        {
            Debug.Log("G1 Quest hand UDP #" + packet_count + ": " + json_text);
        }
    }
    ...
    public Vector3 OperatorToRobot(Vector3 operator_delta)
    {
        ...
        Vector3 robot_delta = new Vector3(
            operator_delta.z,
            -operator_delta.x,
            operator_delta.y);
    ...
    public static bool ShouldDisengageForWorkspace(
        bool local_workspace_exit,
        bool backend_workspace_exit,
        bool backend_feedback_armed)
    {
        return local_workspace_exit
            || (backend_feedback_armed && backend_workspace_exit);
    }

    public static bool GetCommandValidity(
        bool calibrated,
        bool tracking_valid)
    {
        // A brief tracking dropout freezes the last target. It does not
        // release teleoperation; an idle packet tells the backend to hold.
        // Workspace exit is the only state that releases the clutch.
        return calibrated && tracking_valid;
    }

    public static float UpdateWorkspaceExitDuration(
        bool workspace_exit,
        ...
        float delta_time)
    {
        ...
        return Mathf.Max(0.0f, current_duration)
            + Mathf.Max(0.0f, delta_time);
    }

    public static bool IsWorkspaceExitConfirmed(
        float workspace_exit_duration,
    ...
    {
        if (udp_client == null || udp_endpoint == null)
        {
            return false;
        }
    ...
    private void ShutdownSocket()
    {
        ...
        // Closing the socket is a hold condition. A final invalid packet used
        // to look identical to a workspace exit and could release the clutch.
        CloseSocket();
    }
