...
## 작업 장치 정정 (2026-09-18 사용자 확인)

처음부터 현재까지 모든 작업은 같은 노트북에서 진행했다. 별도 데스크톱 PC에서
추가 작업은 아직 진행하지 않았다. `C:/Users/user/Desktop/...`의 `Desktop`은
이 노트북의 바탕화면 폴더명이다. 아래 소스 작업본과 실행 프로젝트는 같은 장치에
있는 서로 다른 폴더이며, 미반영 상태는 장치 이전이 아니라 폴더 간 코드 반영 문제다.

## 현재 노트북에서 실행

**소스 작업본만 수정했다. 같은 노트북의 바탕화면 실행 폴더에는 아직 복사하지 않았다.**
바탕화면 실행 폴더의 기존 BAT를 그대로 쓰면 이번 실행기 수정이 적용되지 않는다.
Unity는 같은 노트북의 기존 실행 프로젝트 SampleScene을 유지하고, Python backend만
