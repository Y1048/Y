## 2026-09-22 두 번째 속도 실험 설정

사용자 요청으로 양팔 목표 접근 rate 상한 3 → 5 s^-1. 각가속도 90°/s² 유지.
오차 기반 rate 감소, 각속도 90/180°/s 및 G1 수신부 제한 유지. PC IK 재시작 후 적용.
실제 로봇 실행 없음. 변경 전 설정과 당일 PC/G1 로그는 Videos/로그1에 보관.

## 2026-09-22 사용자 요청 속도 실험 설정

양팔 목표 접근 rate 상한을 1 → 3 s^-1, 공유 관절 각가속도를 60 → 90°/s²로 변경.
가속도 설정은 tracking과 기본자세 복귀에 함께 적용된다. 오차 기반 rate 감소 및
충돌/관절 제한은 유지된다. 최고 각속도 90/180°/s와 G1 수신부는 변경하지 않았다.
적용하려면 기존 PC Python IK 프로세스를 종료하고 런처를 다시 시작해야 한다.
실제 로봇 실행은 하지 않았다.

## 2026-09-22 상체 Omni 독립 및 양손 회전 축 수정

Omni 수신 freshness를 초기 HMD 정렬/양팔 인게이지/IK 조건에서 제거했다.
샘플 미수신 시 yaw=0, 이후 마지막 값을 유지하며 정지 후 첫 변화도 적용한다.
손목은 G1의 손가락/엄지 축에 맞춰 `LookRotation(finger_direction, palm_across)`로
구성하고, 화면 target과 IK가 같은 회전을 사용한다. 원본 뼈 회전은 raw 진단 필드에 보존.
하체 movement/yaw-rate 및 로봇 수신부 변경 없음. 실제 로봇/Unity Play 실행 없음.
검증과 사람 확인 항목은 `docs/validation/upper_body_omni_hand_frame_20260922.md` 참조.
아래 과거 항목의 measured-yaw/display 변환 설명은 현재 작업본의 동작을 나타내지 않는다.
...
## 2026-09-21 Both arms default and teleop UI wording
Existing-scene setup now configures both arms before saving. Existing laptop SampleScene already has useExistingScene=1/armMode=1; verified and preserved. Menu Use Both Arms shows selected check mark and no Simulation suffix. Input HUD is BIMANUAL IK INPUT; backend startup/restart wording updated. Internal simulation schema/provenance retained for compatibility; measured-state/fallback labels remain accurate. No new motor control. Full runtime + Editor C# compile passed with existing CS1701 warnings (first compiler invocation lacked /noconfig, corrected). Runtime4 files backed up and installed after Play stop confirmation; source installer/runtime scene checks PASS. Unity visual confirmation pending.


## 2026-09-21 Unity exit investigation (not resolved)
User reports BAT-only trigger. Preserved laptop logs under logs/diagnostics/unity_exit_20260921_1816. Windows event18:09:54: Unity.dll access violation0xc0000005 offset0x43fbbb0. Editor-prev: XR no rendered frames, OnApplicationQuit, Meta Interaction OnDisable/UniqueIdentifier.Release exceptions, Oculus display stop, native crash. This locates failure during XR teardown; cause of teardown and link to BAT not established. Launcher reviewed: no Unity process termination/mode change; own worker cleanup only. Do not claim fixed or infer a compiler failure. Opened Unity without Play while existing observation workers remain; scene load completed (PID35192). Operator asked to test Play without rerunning BAT; pending. No SDK settings/package changes or motor program launched. Prior both-arms UI commit c9b19b8 successfully pushed after HTTP/1.1 retry; previous HTTPS attempts reset.


## 2026-09-21 Observation manager process lifetime fixed
Confirmed closed CMD windows left Omni, headless IK, send-live and LowState reader process families alive. Those four exact laptop observation trees were stopped after checking path/role; no motor controller or Unity process terminated. Cause of Unity XR native crash remains unresolved; orphan workers are not proven causal.
New g1_process_lifetime binds the dedicated quiet supervisor to a non-inheritable Windows Job Object with KILL_ON_JOB_CLOSE BEFORE any workers are created. OS exit/forced window closure kills future descendants including venv redirectors/SSH. Job handle is deliberately retained until supervisor exit, not manually closed while self is a member. Job setup failure prevents startup. Existing reused workers are outside the new job and remain untouched. Newly launched camera now belongs to same manager. Enter/Ctrl+C additionally kills each owned child tree; X/manager crash uses OS job cleanup. Diagnostic --show-consoles still uses independent visible windows.
Actual Windows offline process fixtures verified normal and forced supervisor exit kill child/grandchild, unrelated process survives. First fixture used inherited console handles and timed out; corrected to production DEVNULL/CREATE_NO_WINDOW, then all passed. Tests:23 passed /20 subtests; no G1 SDK/SSH/motor execution for tests. Runtime3 Python files backed up/hash-verified; logs retained, no automatic deletion. This supersedes earlier instruction that X leaves workers behind. Unity crash fix not claimed.


## 2026-09-21 Lower camera transport and quiet log load
User reports intermittent G1 input receipt after a reported robot jump; causal link not established. SSH camera now requests/sends at most2fps instead of20fps, retains original JPEG (no new G1 decode/reencode CPU cost), shortens Unity local socket timeout to0.2s, drains/discards frames while Unity unavailable rather than sleeping and building backlog. Hidden sender display/log is1Hz instead of100Hz; actual compute/send remains60Hz and full input schema unchanged. At observed median231KB/frame,2fps estimates3.7Mbps payload vs36.9Mbps at20fps; actual post-change bandwidth/CPU/receipt not measured.27 offline tests/18 subtests passed; no G1 execution/motor tests. Runtime2 files backed up/hash-verified. Existing running workers need manager restart to load changes; do not reproduce robot motion to test network. G1 upper-controller receipt and missed-control-deadline logs still required. No auto-log deletion.


## 2026-09-21 Body compensation drift correction
Latest18:55 operator log showed green/cyan separation ~18cm left/~23cm right; right estimated body translation(.214,-.083,.066)m persisted while head displacement ~2cm. Existing common-motion step integration accepted some outward steps but missed return steps, allowing accumulated bias. Bound candidate compensation to its projection onto current head displacement from engagement anchor, fraction[0,1]. At anchor it is zero; head-only motion from zero estimate cannot create opposite arm input. This remains a heuristic, not measured torso translation; real body displacement can intentionally separate raw wrist and body-relative target. No marker-only correction, IK/gain/speed or motor controller changes.
Production C# math tests executed: hand-only, head-only, common translation, missed-classifier return,1000 cycles, opposite/perpendicular bounds, recorded failure-value bound. Full Unity runtime+Editor C# compile PASS (existingCS1701). Committed reproducible Python compiler harness1PASS (first harness decode failed undercp949; correctedUTF8). No live robot movement or physical verification. After user Play-stop confirmation, one runtime binder file backed up/hash-verified and installed. Operator confirmation of screen alignment pending.


## 2026-09-21 Empty venv prevention
Desktop reported websocket-client missing in pip/setuptools-only .venv-teleop. Existing BAT checked python.exe existence, insufficient; pip check also succeeds with no application packages. START_G1_VR_TELEOP now runs stdlib dependency gate before launcher/network/SSH: isolated project Python3.11x64, all exact requirement versions and imports, pip check. Missing environment/packages auto-installed only into local venv then revalidated; failed/interrupted repair blocks workers; healthy env skips pip install; --check-only never repairs. Windows file lock serializes repairs; lock file is not a success marker. Manual setup also probes complete requirements. Python launcher3.11 and package download access required where missing; no universal offline-install claim.
Actual temporary pip/setuptools-only venv: pip check PASS, new probe FAIL correctly names websocket-client/mujoco.37 tests/20 subtests PASS (mocked repair/error branches plus real bare-venv test). Existing canonical complete environment --check-only PASS. Runtime3 files backed up/hash-verified. No actual fresh-machine pip download, G1 login, workers or motors started. Desktop must pull this commit before new gate takes effect.


## 2026-09-21 Remove default SSH camera's stale WSL prerequisite
Desktop found mirrored mode already enabled but camera WSL environment absent. Actual code still ran WSL pgrep/default preflight and camera/setup check-only incorrectly fell through into legacy WSL setup. Fixed default integrated launcher inventory to Windows camera-launcher rows only; requires ssh.exe, no wsl.exe/WSL process probe. SSH camera check/setup only validate local SSH client (no remote images/SDK). SETUP defaults to SSH; only --camera-transport wsl configures/checks WSL. Explicit camera --transport wsl retained. No .wslconfig mutation/default WSL launch.39 tests/20 subtests PASS including WSL-absent preflight, default check/setup never calls WSL/robot, explicit legacy WSL delegation. Actual local camera --check-only and full SETUP --check-only PASS without network/robot startup.4 runtime files backed up/hash-verified. Desktop must pull; camera/hardware operation not tested.


## 2026-09-21 Unity Omni heading sign correction
Operator observed that an actual G1 right turn rotated the Unity view left. The Omni/G1 yaw convention is positive for a left turn, while Unity positive Y rotation turns the rendered forward direction right. `G1OmniBodyHeading` had applied the Omni delta without converting between those conventions. The Unity-only view delta is now negated through the tested `G1OmniHeadingState.ToUnityYawDelta` conversion. Omni gateway values, `vx/vy/yaw_rate`, arm targets, G1 transport and motor-control paths are unchanged.

Attached `send.log` confirms observation packets and G1 audit ACKs were exchanged, but that ACK proves receipt by the audit receiver only. `lowstate.log` contains a path to the full LowState JSONL rather than the samples, so this turn did not reconstruct measured base yaw from LowState. The direction defect is grounded in the operator's physical observation and the mismatched sign in the Unity transform.

Verification: 22 heading-state/sign assertions PASS; full Unity runtime script compilation PASS with the existing Newtonsoft/netstandard CS1701 warning. The two changed Unity runtime files were installed only after confirming the active Desktop copies exactly matched the previous Git HEAD. No G1 process, SDK/DDS publisher or motor command was run. Quest visual confirmation remains pending.


## 2026-09-22 Separate Omni operator yaw, measured G1 yaw and hand input
The prior view-only sign fix still rotated the shared XR TrackingSpace directly from Omni heading. Because Quest head and hand transforms share that space, command-to-robot yaw lag could enter the IK input coordinates. The heading component now computes the display correction as `measured G1 base yaw delta - operator Omni body yaw delta`. When the operator turns first, this cancels the operator turn until LowState shows that G1 has turned; as measured yaw catches up, the view follows it. The ambient world is no longer rotated by the command heading.

Both hand binders now remove the displayed/measured body yaw from wrist, anatomical finger, palm and head poses before engagement and target-delta calculations. The engagement heading is captured from the corrected head rotation. Thus the render-space correction and robot response lag do not become arm movement. If measured base state is unavailable, the display transform is left untouched and Omni yaw is still removed from the hand input frame. This does not time-align asynchronous Omni, Quest and LowState samples and actual visual comfort still requires a combined Quest+Omni+G1 check.

Verification: 25 production heading-state/correction assertions PASS; full Unity runtime compilation PASS with the existing CS1701 warning; body-compensation and Unity workspace policy tests 14/14 PASS. The four changed runtime scripts were installed only after their prior Desktop copies matched Git HEAD, then content-matched to the canonical source (one file retains its runtime line endings). No G1 SDK/DDS publisher, SSH command or motor output was run.


## 2026-09-22 START_G1_VR_TELEOP Unity editor launch
`tools/START_G1_VR_TELEOP.bat` now opens this checkout's `Unity_G1_VR` project with the pinned Unity `6000.5.4f1` editor after normal preflight/login and before the quiet observation manager binds its worker job. An already-open editor for the exact project is reused; another Unity project does not count, and duplicate editors for this project fail closed. Unity remains outside the observation manager's kill-on-close job, so stopping the manager does not close the user's editor. The launcher opens the Editor only; it never enters Play automatically.

`--check-only` validates the Unity project/version/editor path without starting Unity or workers. `--no-unity` keeps the prior observation-only behavior. The active Desktop launcher matched the previous Git HEAD before the narrow replacement. Offline launcher/dependency tests: 32 passed plus 20 subtests. Local read-only resolution found `C:\Program Files\Unity\Hub\Editor\6000.5.4f1\Editor\Unity.exe`; no editor, SSH worker, camera, G1 SDK/DDS or motor command was started for verification.
