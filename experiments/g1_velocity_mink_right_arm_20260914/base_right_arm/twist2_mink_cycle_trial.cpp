...
class Controller {
 ...
 private:
  ...
  void write_cycle() {
    ...
    for (std::size_t i = 0; i < command.motor_cmd().size(); ++i) {
      ...
      const float torque_without_position =
          -kd_[i] * state.motor_state()[i].dq() + feedforward;
      const float lower_from_torque =
          state.motor_state()[i].q() +
          (-soft_torque - torque_without_position) / kp_[i];
      ...
      const double predicted_torque =
          kp_[i] * (target - state.motor_state()[i].q()) -
          kd_[i] * state.motor_state()[i].dq() + feedforward;
  ...
  const std::array<float,kDofs> kp_,kd_;
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    std::cout<<"[CYCLE] profile="<<selected_profile<<"; measured speed guard = selected right-arm cap + 0.8 rad/s; pinch returns; hard stops still damping\n";
    ...
    const bool pd_trial = argc>=7 && std::string(argv[6])=="--pd-reach-trial";
    const bool keyboard_right_arm = argc>=7 && std::string(argv[3])=="--enable-actuation" &&
        std::string(argv[4])=="--policy-seconds" && (std::string(argv[6])=="--udp-right-arm" || pd_trial);
    if(!keyboard_right_arm)throw std::runtime_error("Use --enable-actuation --policy-seconds 300 with --udp-right-arm OR --pd-reach-trial");
    if(pd_trial)throw std::runtime_error("cycle candidate supports UDP only; use preserved binary for PD trials");
    const auto gains=ParsePdGainOptions(argc,argv,kKp,kKd);
    ...
    if (pd_trial) {
      std::cout<<"[PD TRIAL] Move both arms to Mink ready pose, then full forward reach/return, 3 cycles. No UDP input. Fixed reference for PD comparison; speed cap45 deg/s, acceleration cap10 rad/s^2; dq command stays zero. p/R1/Select/B stop.\n";
    ...
    if (keyboard_right_arm && !pd_trial) {
      std::cout
          << "UDP timeline: blend, both-arm initial pose, then absolute right-arm input; 300 s total policy\n";
      std::cout<<"[UDP] Cycle candidate: explicit yesterday/today profile; pinch returns, measured-ready ACK permits re-engage; R1/Select/B/p still damping. Wait for udp_ready.\n";
    }
    std::cout
    ...
    const nlohmann::json run_manifest={
      {"schema","g1.pd.run.v1"},{"mode",pd_trial?"pd_reach":"udp"},
    ...
    if (keyboard_right_arm) {
      ...
      if(!pd_trial)std::cout<<"[UDP] Cycle candidate: explicit yesterday/today profile; pinch returns, measured-ready ACK permits re-engage; R1/Select/B/p still damping. Wait for udp_ready.\n";
    }
    ...
    auto udp_target=std::make_shared<MinkLiveCycleTarget>(baseline,clock(),clock,!pd_trial);
    std::unique_ptr<NativeVrCycleUdp> udp;
    std::unique_ptr<PdReachTrial> trial;
    PdReadySettle pd_settle;
    double trial_started=-1;
    if(!pd_trial) {
      udp=std::make_unique<NativeVrCycleUdp>();
    ...
    try {
      ...
      const LowState state = controller.snapshot(&state_age_ms,&received);
        ...
        if (keyboard_right_arm) {
          for(const char key:keyboard->read_available())if(key=='p'||key=='P')controller.latch("keyboard stop requested",true);
          if(controller.damping())break;
            ...
            if(!trial){
              ...
              if(udp_target->Ready()){
                ...
                if(pd_settle.Update(clock(),receipt,measured_q,measured_dq,applied)){
                  trial=std::make_unique<PdReachTrial>(applied);trial_started=elapsed;
                }
              }
            }
            if(trial){
              const auto point=trial->At(elapsed-trial_started);
              applied=point.q;trial_phase=point.phase;trial_cycle=point.cycle;
              if(point.phase==6){controller.latch("pd trial completed",true);break;}
          ...
          if(!pd_trial&&alpha>=1.0F)phase=udp_target->Phase();
        }
        std::array<float, kDofs> mimic_target{};
        for (std::size_t i = 0; i < kDofs; ++i) {
    ...
    csv.Finish();
    {
      const nlohmann::json result={{"schema","g1.pd.result.v1"},
        ...
        {"completed_reach",pd_trial&&controller.reason()=="pd trial completed"},
    ...
    const bool completed_duration =
        (controller.reason() == "planned policy duration completed" || controller.reason() == "pd trial completed");
