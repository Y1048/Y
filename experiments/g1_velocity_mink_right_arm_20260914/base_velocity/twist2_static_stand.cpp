...
#ifdef G1_VELOCITY_TWIST2_STOP
namespace twist2 {
using StaticStandPolicy = Policy;
using StaticStandObservationHistory = ObservationHistory;
using StaticStandPolicyResult = PolicyResult;
constexpr char kStaticStandExpectedPolicySha256[] =
    "463be0376c2c1f551b996d0bf9ab97833854f2cc098b9d4fea735f17ec2e9015";
inline const std::array<float, kDofs>& static_stand_default() {
  return kDefault;
}
inline std::array<float, kDofs> static_stand_hybrid_target(
    const std::array<float, kDofs>& action,
    const std::array<float, kDofs>& upper_target) {
  auto target = hybrid_target(action, upper_target);
  constexpr float kStaticLegCommandMargin = 0.08F;
  constexpr float kStaticAnkleRollCommandMargin = 0.12F;
  for (std::size_t i = 0; i < kLegDofs; ++i) {
    const float margin =
        (i == 5U || i == 11U)
            ? kStaticAnkleRollCommandMargin
            : kStaticLegCommandMargin;
    target[i] = std::clamp(
        target[i], kLower[i] + margin, kUpper[i] - margin);
  }
  return target;
}
inline std::array<float, kDofs> static_stand_target_as_action(
    const std::array<float, kDofs>& target) {
  return target_as_action(target);
}
}  // namespace twist2
#endif
#ifdef G1_VELOCITY_POLICY
#include "g1_velocity_policy.hpp"
#define Policy VelocityPolicy
#define ObservationHistory VelocityObservationHistory
#define PolicyResult VelocityPolicyResult
#define hybrid_target velocity_hybrid_target
#define target_as_action velocity_target_as_action
#define kDefault kVelocityDefault
#define kExpectedPolicySha256 kVelocityPolicySha256
#endif
...
#ifdef G1_VELOCITY_POLICY
constexpr auto kHandoffCommandGrace = std::chrono::milliseconds(1000);
#else
...
#ifdef G1_VELOCITY_TWIST2_STOP
constexpr double kVelocitySwitchBlendSeconds = 4.0;
constexpr double kStaticStandSwitchBlendSeconds = 6.0;
constexpr float kStaticBlendAttitudeLimit = 0.25F;
constexpr char kStaticStandPolicyPath[] =
...
const char* leg_policy_mode_name(LegPolicyMode mode) {
  switch (mode) {
...
constexpr std::uint16_t kX = 0x0400U;
...
void print_keyboard_help() {
  std::cout
#ifdef G1_VELOCITY_POLICY
      << "\n[keyboard] locomotion (latched; R1 is the deadman):\n"
      << "  8 forward +0.40 m/s    2 backward -0.20 m/s\n"
      << "  4 left    +0.20 m/s    6 right    -0.20 m/s\n"
      << "  7 yaw     +0.20 rad/s  9 yaw      -0.20 rad/s\n"
#ifdef G1_VELOCITY_TWIST2_STOP
      << "  5 stop: two zero-command gait cycles, then 6 s TWIST2 blend\n"
      << "  a direction key blends back to velocity control in 4 s\n"
#elif defined(G1_VELOCITY_CONTINUOUS_GAIT)
      << "  5 stop: velocity becomes zero; gait phase keeps running\n"
#else
      << "  5 stop: decelerate, then hold the next double-support phase\n"
      << "  a direction key restarts gait from the held phase\n"
#endif
#endif
      ...
      << "  wrist yaw      u/j/m\n"
#ifdef G1_VELOCITY_POLICY
      << "[keyboard] arm speed is limited to 0.08 rad/s; "
...
class Controller {
 public:
  ...
  void validate_state(
    ...
    for (std::size_t i = 0; i < kDofs; ++i) {
      ...
      if (motor.q() < kLower[i] + kJointLimitMargin ||
        ...
        throw std::runtime_error(
            "joint position soft limit: index=" + std::to_string(i) +
            " q=" + std::to_string(motor.q()) + " allowed=[" +
            std::to_string(kLower[i] + kJointLimitMargin) + "," +
            std::to_string(kUpper[i] - kJointLimitMargin) + "]");
      }
  ...
  void wait_for_preflight() {
    std::cout
        << "[preflight] AI standing must be active; hold "
#ifdef G1_VELOCITY_POLICY
           "R1+X continuously; "
#else
           "R1 continuously; "
...
#ifdef G1_VELOCITY_POLICY
        if ((remote_buttons(state) & kX) == 0U) {
          throw std::runtime_error("R1+X start combination not held");
        }
#endif
...
#ifdef G1_VELOCITY_POLICY
    int fsm_id = -1;
    const std::int32_t fsm_result = loco_->GetFsmId(fsm_id);
    if (fsm_result != 0 || fsm_id != 501) {
      throw std::runtime_error(
          "Expected Regular FSM 501 before ReleaseMode; result=" +
          std::to_string(fsm_result) + " fsm_id=" +
          std::to_string(fsm_id));
    }
    std::cout << "[handoff] verified Regular FSM 501\n";
#endif
 ...
 private:
  ...
  void write_cycle() {
    ...
    if (!damping_.load()) {
#ifdef G1_VELOCITY_POLICY
      if ((remote_buttons(state) & kR1) == 0U) {
        latch("R1 released", true);
      }
#endif
    }
    if (!damping_.load()) {
...
std::filesystem::path write_csv(const std::vector<Sample>& samples) {
...
#ifdef G1_VELOCITY_POLICY
#ifdef G1_VELOCITY_TWIST2_STOP
  name << "g1_velocity_twist2_stop_" << timestamp << ".csv";
#else
  name << "g1_velocity_keyboard_release_" << timestamp << ".csv";
#endif
#else
  ...
  stream
      << "elapsed_s,phase,alpha,arm_alpha,state_age_ms,inference_ms,"
         "roll_rad,pitch_rad,cmd_vx,cmd_vy,cmd_yaw,gait_phase,"
         "gait_running";
  for (std::size_t i = 0; i < kDofs; ++i) {
  ...
  for (const Sample& sample : samples) {
    stream << sample.elapsed_s << ',' << sample.phase << ',' << sample.alpha
           ...
           << sample.pitch << ',' << sample.command[0] << ','
           << sample.command[1] << ',' << sample.command[2] << ','
           << sample.gait_phase << ',' << (sample.gait_running ? 1 : 0);
    for (std::size_t i = 0; i < kDofs; ++i) {
  ...
  try {
    ...
    if (!default_duration && !explicit_duration && !left_arm_motion &&
      ...
      std::cerr
          ...
          << " eth0 /absolute/"
#ifdef G1_VELOCITY_POLICY
             "g1_velocity_12dof_motion.pt "
#else
             "twist2_1017_20k_torchscript.pt "
#endif
...
#ifdef G1_VELOCITY_TWIST2_STOP
    const std::filesystem::path static_stand_policy_path(
        kStaticStandPolicyPath);
    StaticStandPolicy static_stand_policy{static_stand_policy_path};
#endif
    const std::string confirmation_phrase =
#ifdef G1_VELOCITY_POLICY
        "VELOCITY-LOWCMD";
#else
        "P";
...
#ifdef G1_VELOCITY_POLICY
#ifdef G1_VELOCITY_TWIST2_STOP
        << "=== G1 VELOCITY + TWIST2 STATIC STOP / RELEASEMODE ===\n"
#elif defined(G1_VELOCITY_CONTINUOUS_GAIT)
        << "=== G1 12-DOF VELOCITY / CONTINUOUS GAIT BACKUP ===\n"
#else
        << "=== G1 12-DOF VELOCITY / RELEASEMODE LOWCMD CANDIDATE ===\n"
#endif
#else
...
#ifdef G1_VELOCITY_TWIST2_STOP
        << "static stand policy: " << static_stand_policy_path << '\n'
        << "static stand sha256: "
        << kStaticStandExpectedPolicySha256 << '\n'
        << "keyboard leg modes: TWIST2 stand -> direction key -> 4 s "
           "velocity blend; 5 -> two zero-command gait cycles -> 6 s "
           "TWIST2 blend\n"
#endif
        << "timeline: capture 1 s -> leg-only blend 4 s -> "
#ifdef G1_VELOCITY_POLICY
#ifdef G1_VELOCITY_TWIST2_STOP
           "TWIST2 stand and velocity legs "
#else
           "velocity legs "
#endif
#else
    ...
    if (keyboard_left_arm) {
      std::cout
          << "keyboard timeline: after blend, "
#ifdef G1_VELOCITY_POLICY
             "locomotion and left arm 7-DOF are interactive for 300 s\n"
#else
             "left arm 7-DOF is interactive for 300 s\n"
#endif
#ifndef G1_VELOCITY_POLICY
    ...
    std::cout
        << "Hold R1 continuously; release R1 or press Select/B for immediate "
           "damping.\n"
...
#ifdef G1_VELOCITY_TWIST2_STOP
    StaticStandObservationHistory static_readiness_history;
#endif
...
#ifdef G1_VELOCITY_TWIST2_STOP
      const StaticStandPolicyResult static_result =
          static_readiness_history.infer(
              static_stand_policy, state, controller.capture_q());
      const auto static_target = static_stand_hybrid_target(
          static_result.action, controller.capture_q());
      static_readiness_history.commit(
          static_result.current,
          static_stand_target_as_action(static_target));
      warmup_ms.push_back(std::max(
          result.inference_ms, static_result.inference_ms));
#else
...
#ifdef G1_VELOCITY_TWIST2_STOP
    static_readiness_history.reset();
    const StaticStandPolicyResult static_readiness =
        static_readiness_history.infer(
            static_stand_policy, readiness_state, controller.capture_q());
#endif
    if (readiness.inference_ms > 45.0
#ifdef G1_VELOCITY_TWIST2_STOP
        || static_readiness.inference_ms > 45.0
#endif
    ) {
      throw std::runtime_error(
    ...
    std::cout << "[policy] real-state warm-up passed: final="
              ...
              << " ms"
#ifdef G1_VELOCITY_TWIST2_STOP
              << ", TWIST2 final=" << static_readiness.inference_ms << " ms"
#endif
              << '\n';
...
#ifdef G1_VELOCITY_TWIST2_STOP
    StaticStandObservationHistory static_stand_history;
#endif
...
#ifdef G1_VELOCITY_TWIST2_STOP
    LegPolicyMode leg_policy_mode = LegPolicyMode::kStaticStand;
    auto leg_transition_started = Clock::now();
    std::array<float, kDofs> leg_transition_start = controller.capture_q();
    std::array<float, kDofs> last_desired_target = controller.capture_q();
#endif
    ...
    try {
      ...
      while (!controller.damping() && !gInterrupt.load()) {
        ...
        if ((remote_buttons(state) & kR1) == 0U) {
          ...
          controller.latch("R1 released", true);
          break;
        }
#endif
        controller.validate_state(state, true, kVelocityLimit, false);
#ifdef G1_VELOCITY_TWIST2_STOP
        if (leg_policy_mode == LegPolicyMode::kStaticBlend &&
            (std::abs(state.imu_state().rpy()[0]) >
                 kStaticBlendAttitudeLimit ||
             std::abs(state.imu_state().rpy()[1]) >
                 kStaticBlendAttitudeLimit)) {
          std::ostringstream message;
          message << "TWIST2 transition attitude guard: roll="
                  << state.imu_state().rpy()[0]
                  << " pitch=" << state.imu_state().rpy()[1]
                  << " limit=" << kStaticBlendAttitudeLimit;
            ...
            if (VelocityCommand::IsKey(key)) {
              if (!keyboard_active) {
                ...
                const bool stop_key =
                    key == '5' || key == '0' || key == ' ';
                if (stop_key) {
                  VelocityCommand::HandleKey(key);
                  if (leg_policy_mode == LegPolicyMode::kVelocity) {
                    leg_policy_mode = LegPolicyMode::kVelocitySettle;
                  } else if (
                      leg_policy_mode == LegPolicyMode::kVelocityBlend) {
                    static_stand_history.reset();
                    leg_transition_start = last_desired_target;
                    for (std::size_t i = 0; i < kLegDofs; ++i) {
                      leg_transition_start[i] = state.motor_state()[i].q();
                    }
                    leg_transition_started = now;
                    leg_policy_mode = LegPolicyMode::kStaticBlend;
                  }
                  std::cout
                      << "\n[keyboard] stop requested; zero-command gait "
                         "settle, then TWIST2 stand blend\n";
                } else {
                  if (leg_policy_mode == LegPolicyMode::kStaticStand ||
                      leg_policy_mode == LegPolicyMode::kStaticBlend) {
                    history.reset();
                    VelocityCommand::HandleKey(key);
                    leg_transition_start = last_desired_target;
                    ...
                    VelocityCommand::HandleKey(key);
                    if (leg_policy_mode == LegPolicyMode::kVelocitySettle) {
                      leg_policy_mode = LegPolicyMode::kVelocity;
                    }
                  }
                  std::cout
                      << "\n[keyboard] gait command=" << key
                      << "; leg_mode="
                      << leg_policy_mode_name(leg_policy_mode) << '\n';
                }
#else
                VelocityCommand::HandleKey(key);
                if (key == '5' || key == '0' || key == ' ') {
                  ...
                  std::cout
                      << "\n[keyboard] velocity stop requested; gait phase "
                         "continues\n";
#else
                  std::cout
                      << "\n[keyboard] stop requested; decelerating, then "
                         "freezing at the next double-support phase\n";
#endif
                } else {
                  std::cout
                      << "\n[keyboard] gait command=" << key
        ...
        std::array<float, kDofs> mimic_target{};
        std::array<float, kDofs> selected_action{};
        std::array<float, kDofs> full_policy_target{};
        std::array<float, 3> display_command{};
        float display_gait_phase = 0.0F;
        bool display_gait_running = false;
        double active_inference_ms = 0.0;
        bool velocity_policy_active = true;
#ifdef G1_VELOCITY_TWIST2_STOP
        PolicyResult velocity_result;
        StaticStandPolicyResult static_result;
        velocity_policy_active =
            leg_policy_mode == LegPolicyMode::kVelocityBlend ||
            leg_policy_mode == LegPolicyMode::kVelocity ||
            leg_policy_mode == LegPolicyMode::kVelocitySettle;
        bool begin_static_blend_after_frame = false;
        float switch_alpha = 1.0F;
        if (velocity_policy_active) {
          for (std::size_t i = 0; i < kDofs; ++i) {
          ...
          velocity_result = history.infer(policy, state, mimic_target);
          active_inference_ms = velocity_result.inference_ms;
          selected_action = velocity_result.action;
          display_command = velocity_result.command;
          display_gait_phase = velocity_result.gait_phase;
          display_gait_running = velocity_result.gait_clock_running;
          full_policy_target =
              hybrid_target(velocity_result.action, upper_target);
          begin_static_blend_after_frame =
              leg_policy_mode == LegPolicyMode::kVelocitySettle &&
              !VelocityCommand::MotionRequested() &&
              !velocity_result.gait_clock_running;
        } else {
          for (std::size_t i = 0; i < kDofs; ++i) {
            mimic_target[i] = i < kLegDofs
                                  ? (1.0F - alpha) *
                                            controller.capture_q()[i] +
                                        alpha * static_stand_default()[i]
                                  : upper_target[i];
          }
          static_result = static_stand_history.infer(
              static_stand_policy, state, mimic_target);
          active_inference_ms = static_result.inference_ms;
          selected_action = static_result.action;
          full_policy_target = static_stand_hybrid_target(
              static_result.action, upper_target);
        }
        if (leg_policy_mode == LegPolicyMode::kVelocityBlend ||
            leg_policy_mode == LegPolicyMode::kStaticBlend) {
          const double switch_blend_seconds =
              leg_policy_mode == LegPolicyMode::kStaticBlend
                  ? kStaticStandSwitchBlendSeconds
                  : kVelocitySwitchBlendSeconds;
          switch_alpha = smoothstep(static_cast<float>(
              std::chrono::duration<double>(
                  now - leg_transition_started).count() /
              switch_blend_seconds));
        }
        if (alpha >= 1.0F) {
          phase = leg_policy_mode_name(leg_policy_mode);
        }
#else
        for (std::size_t i = 0; i < kDofs; ++i) {
          mimic_target[i] = i < kLegDofs
        ...
        active_inference_ms = result.inference_ms;
        selected_action = result.action;
        display_command = result.command;
        display_gait_phase = result.gait_phase;
        display_gait_running = result.gait_clock_running;
        full_policy_target = hybrid_target(result.action, upper_target);
#endif
        inference_ms.push_back(active_inference_ms);
        if (active_inference_ms > 45.0) {
...
#ifdef G1_VELOCITY_TWIST2_STOP
          if (i < kLegDofs && alpha >= 1.0F &&
              (leg_policy_mode == LegPolicyMode::kVelocityBlend ||
               leg_policy_mode == LegPolicyMode::kStaticBlend)) {
            desired.target[i] =
                (1.0F - switch_alpha) * leg_transition_start[i] +
                switch_alpha * full_policy_target[i];
          }
#endif
...
#ifdef G1_VELOCITY_TWIST2_STOP
        if (velocity_policy_active) {
          history.commit(velocity_result.current, applied);
        } else {
          static_stand_history.commit(
              static_result.current,
              static_stand_target_as_action(desired.target));
        }
        if (leg_policy_mode == LegPolicyMode::kVelocityBlend &&
            switch_alpha >= 1.0F) {
          leg_policy_mode = LegPolicyMode::kVelocity;
          std::cout << "\n[legs] velocity policy owns legs\n";
        } else if (leg_policy_mode == LegPolicyMode::kStaticBlend &&
                   switch_alpha >= 1.0F) {
          leg_policy_mode = LegPolicyMode::kStaticStand;
          std::cout << "\n[legs] TWIST2 static stand owns legs\n";
        }
        if (begin_static_blend_after_frame) {
          static_stand_history.reset();
          leg_transition_start = desired.target;
          for (std::size_t i = 0; i < kLegDofs; ++i) {
            leg_transition_start[i] = state.motor_state()[i].q();
          }
          leg_transition_started = now;
          leg_policy_mode = LegPolicyMode::kStaticBlend;
          std::cout
              << "\n[legs] zero-command gait settled; starting 6 s "
                 "TWIST2 stand blend\n";
        }
        last_desired_target = desired.target;
#else
        ...
        sample.inference_ms = active_inference_ms;
        sample.roll = state.imu_state().rpy()[0];
        sample.pitch = state.imu_state().rpy()[1];
        sample.command = display_command;
        sample.gait_phase = display_gait_phase;
        sample.gait_running = display_gait_running;
        sample.action = selected_action;
        ...
        for (std::size_t i = 0; i < kDofs; ++i) {
          ...
          if (i < kLegDofs) {
            action_abs_max = std::max(
                action_abs_max,
                std::abs(static_cast<double>(selected_action[i])));
            action_delta_abs_max = std::max(
                action_delta_abs_max,
                std::abs(static_cast<double>(
                    selected_action[i] - previous_action[i])));
          }
        }
        ...
        if (samples.size() % console_period == 0) {
          std::cout << "\r[actuate] " << std::fixed << std::setprecision(2)
                    ...
                    << " arm=" << arm_alpha
#ifdef G1_VELOCITY_POLICY
                    << " cmd=[" << display_command[0] << ','
                    << display_command[1] << ',' << display_command[2] << ']'
                    << " gait="
                    << (display_gait_running ? "run" : "hold")
                    << " gait_phase=" << display_gait_phase
#ifdef G1_VELOCITY_TWIST2_STOP
                    << " leg_owner="
                    << leg_policy_mode_name(leg_policy_mode)
#endif
