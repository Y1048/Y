...
#include "native_velocity_udp.hpp"
...
std::string csv_header() {
  ...
  stream
      << "elapsed_s,phase,alpha,arm_alpha,state_age_ms,inference_ms,"
         "roll_rad,pitch_rad,leg_policy_mode";
  for (std::size_t i = 0; i < kDofs; ++i) {
...
void write_csv_sample(std::ostream& stream, const Sample& sample) {
    ...
    stream << sample.elapsed_s << ',' << sample.phase << ',' << sample.alpha
           ...
           << sample.pitch << ',' << sample.leg_policy_mode;
    for (std::size_t i = 0; i < kDofs; ++i) {
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    if (handoff_only) {
      throw std::runtime_error(
          "--handoff-only-trial is disabled before DDS initialization: "
          "the 2026-09-14 physical probe stopped LowCmd before Regular/AI "
          "ownership was verified and actuator support was lost");
    }
    const bool pd_trial = argc>=7 &&
        (std::string(argv[6])=="--pd-reach-trial"||pd_sweep);
    const bool velocity_axis_trial = argc>=7 &&
        std::string(argv[6])=="--velocity-axis-trial";
    ...
    if(!keyboard_right_arm && !velocity_axis_trial)throw std::runtime_error("Use --enable-actuation --policy-seconds with --udp-right-arm, --velocity-axis-trial, --pd-reach-trial, --pd-sweep-trial, or --handoff-only-trial");
    ...
    if (!default_duration && !explicit_duration && !right_arm_motion &&
        !keyboard_right_arm && !velocity_axis_trial) {
      std::cerr
          << "Usage: " << argv[0]
          ...
          << " eth0 /absolute/twist2_1017_20k_torchscript.pt "
             ...
             "--udp-right-arm\n"
          << "       " << argv[0]
          << " eth0 /absolute/velocity_policy.pt "
             "--enable-actuation --policy-seconds 30..300 "
             "--velocity-axis-trial\n";
      return 2;
    }
    double policy_seconds = kDefaultPolicySeconds;
    if (explicit_duration || right_arm_motion || keyboard_right_arm || velocity_axis_trial) {
      std::size_t parsed = 0;
      ...
      if (parsed != duration_text.size() ||
        ...
        throw std::runtime_error(
            (keyboard_right_arm || velocity_axis_trial)
                ? "long-running mode requires --policy-seconds from 2 through 300"
    ...
    if (velocity_axis_trial &&
        (policy_seconds < 30.0 || policy_seconds > kKeyboardPolicySeconds)) {
      throw std::runtime_error(
          "velocity axis trial requires --policy-seconds from 30 through 300");
    }
    ...
    std::cout
        << "=== G1 VELOCITY + MINK RIGHT ARM TRIAL ===\n"
        << "policy: " << policy_path << '\n'
        ...
        << "timeline: capture 1 s -> leg-only blend 4 s -> "
           "velocity-policy legs " << policy_seconds
        << " s -> safe-waypoint return -> full-body position hold\n";
    if (handoff_only) {
      std::cout<<"[STATIC STAND HANDOFF PROBE] Capture for 1 s, blend the legs into the TWIST2 static-stand policy for 4 s, then stop the LowCmd writer and request verified Regular ai/FSM handoff. Upper-body targets remain captured; no arm trajectory, keypad command, or UDP input.\n";
    } else if (pd_trial) {
      if(pd_sweep)
      ...
      std::cout
          << "control: TWIST2 static stand owns idle legs 0:12; Unity keypad "
             "switches them to the velocity policy; waist held; "
             "right arm 7-DOF absolute Mink UDP; both arms initialize to Mink ready pose\n";
    } else if (velocity_axis_trial) {
      std::cout
          << "[VELOCITY AXIS TRIAL] TWIST2 static stand owns idle legs; "
             "token-bound UDP velocity requests switch to the velocity policy. "
             "Waist and both arms remain at captured targets; no VR/Mink input.\n";
    } else {
      std::cout
    ...
    for (int step = 0; step < 20; ++step) {
      ...
      warmup_ms.push_back(
          std::max(result.inference_ms, static_result.inference_ms));
    ...
    const double readiness_inference_ms =
        std::max(readiness.inference_ms, static_readiness.inference_ms);
    if (readiness_inference_ms > 45.0) {
      throw std::runtime_error(
          "readiness inference exceeded 45 ms before handoff");
    }
    std::cout << "[policy] real-state warm-up passed: final="
              << std::fixed << std::setprecision(3)
              << readiness_inference_ms << " ms, warmup_max="
    ...
    // The deployed velocity policy was trained with a continuously advancing
    // gait clock.  Keep it as the leg owner even for a zero velocity command;
    // replacing it with a captured pose removes active balance control.
    LegPolicyMode leg_policy_mode = LegPolicyMode::kVelocity;
#else
    ...
    const nlohmann::json run_manifest={
      ...
      {"static_stand_policy_sha256",kStaticStandExpectedPolicySha256},
    ...
    std::unique_ptr<NativeVelocityUdp> velocity_udp;
    if (!pd_active) velocity_udp = std::make_unique<NativeVelocityUdp>();
    ...
    if(!pd_active && !velocity_axis_trial)enable_udp();
    ...
    try {
      ...
      const LowState state = controller.snapshot(&state_age_ms,&received);
        ...
        if (velocity_udp) {
          velocity_udp->Drain();
          // Locomotion is independent of arm engagement. Suppress walking only
          // during the initial full-body takeover blend.
          if (alpha < 1.0F) {
            VelocityCommand::Stop();
          }
        }

        const bool velocity_requested =
            velocity_udp && VelocityCommand::MotionRequested();
#ifdef G1_VELOCITY_CONTINUOUS_GAIT
        // Ownership is continuous.  velocity_requested still represents the
        // operator command and remains false for the zero-command gait.
        const bool velocity_policy_requested = true;
#else
        const bool velocity_policy_requested = velocity_requested;
#endif
        const auto capture_leg_transition_start = [&]() {
        ...
        const LegPolicyTransition request_transition =
            requested_transition(leg_policy_mode, velocity_policy_requested);
        switch (request_transition) {
          case LegPolicyTransition::kStartVelocityBlend:
            history.reset();
            capture_leg_transition_start();
            std::cout << "\n[legs] keypad motion requested; starting 4 s "
                         "TWIST2-to-velocity blend\n";
            break;
          case LegPolicyTransition::kStartStaticBlend:
            static_stand_history.reset();
            capture_leg_transition_start();
            std::cout << "\n[legs] motion cancelled during blend; starting "
                         "6 s return to TWIST2 stand\n";
            break;
          default:
            break;
        }
        leg_policy_mode =
            apply_transition(leg_policy_mode, request_transition);

        const bool velocity_active =
            velocity_policy_active(leg_policy_mode);
        ...
        if (velocity_active) {
          ...
          begin_static_blend_after_frame =
              after_velocity_frame_transition(
                  leg_policy_mode, velocity_policy_requested,
        ...
        if (leg_policy_mode == LegPolicyMode::kVelocityBlend ||
          ...
          switch_alpha = smoothstep(static_cast<float>(
              std::chrono::duration<double>(
                  now - leg_transition_started).count() / switch_seconds));
        }
        if (leg_policy_mode == LegPolicyMode::kStaticBlend &&
            (std::abs(state.imu_state().rpy()[0]) >
             ...
             std::abs(state.imu_state().rpy()[1]) >
                 kStaticBlendAttitudeLimit)) {
          throw std::runtime_error("static-stand transition attitude limit");
        }
        ...
        const LegPolicyTransition completed_transition =
            completed_blend_transition(
                leg_policy_mode, switch_alpha >= 1.0F);
        if (completed_transition ==
            LegPolicyTransition::kFinishVelocityBlend) {
          std::cout << "\n[legs] velocity policy owns legs\n";
        } else if (completed_transition ==
                   LegPolicyTransition::kFinishStaticBlend) {
          std::cout << "\n[legs] TWIST2 static stand owns legs\n";
        }
        leg_policy_mode =
            apply_transition(leg_policy_mode, completed_transition);
        if (begin_static_blend_after_frame) {
          static_stand_history.reset();
          last_desired_target = desired.target;
          capture_leg_transition_start();
          leg_policy_mode = apply_transition(
              leg_policy_mode, LegPolicyTransition::kStartStaticBlend);
          std::cout << "\n[legs] zero-command gait settled; starting 6 s "
                       "velocity-to-TWIST2 stand blend\n";
        }
        last_desired_target = desired.target;
        ...
        sample.phase = phase;
        sample.leg_policy_mode = leg_policy_mode_name(leg_policy_mode);
        sample.alpha = alpha;
