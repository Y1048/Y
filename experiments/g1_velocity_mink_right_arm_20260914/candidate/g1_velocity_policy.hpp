...
class VelocityCommand {
 public:
  static void SetTarget(const std::array<float, 3>& next) {
    constexpr std::array<float, 3> kAbsLimit{0.80F, 0.80F, 0.80F};
    for (std::size_t i = 0; i < next.size(); ++i) {
      if (!std::isfinite(next[i]) || std::abs(next[i]) > kAbsLimit[i] + 1.0e-6F) {
        throw std::runtime_error("Unity velocity target outside policy limits");
      }
    }
    target_ = next;
  }
...
inline std::array<float, kDofs> velocity_hybrid_target(
  ...
  for (std::size_t i = 0; i < kDofs; ++i) {
    const float margin = i < kLegDofs
        ? ((i == 5U || i == 11U) ? 0.12F : 0.08F)
        : kJointLimitMargin;
    target[i] = std::clamp(
