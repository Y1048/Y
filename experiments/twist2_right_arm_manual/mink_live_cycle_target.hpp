...
public:
 ...
 C::Q Update(double,bool blend_complete){std::lock_guard<std::mutex> lock(mutex_);double now=clock_();
  ...
  if(!cycle_){
   ...
   for(size_t i=15;i<29;++i)settled=settled&&C::MeasuredReadyJoint(feedback_.q[i],feedback_.dq[i],target_[i],(i-15)%7);
   if(!settled)settle_start_=-1;else if(settle_start_<0)settle_start_=now;
   if(settle_start_>=0&&now-settle_start_>=.5){
 ...
 J Acknowledgement(const std::string& token){std::lock_guard<std::mutex> lock(mutex_);
  ...
  if(!cycle_){
   ...
   for(size_t i=15;i<29;++i){
    ...
    if(!C::MeasuredReadyJoint(feedback_.q[i],feedback_.dq[i],target_[i],(i-15)%7))
     blockers.push_back(J{{"joint",i},{"error_rad",error},{"limit_rad",C::ready_error[(i-15)%7]},{"speed_rad_s",rate},{"speed_limit_rad_s",.05}});
   }
