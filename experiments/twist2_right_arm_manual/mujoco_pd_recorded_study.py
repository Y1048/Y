"""Finite recorded-target stress comparison, OFFLINE ONLY.
...
def summarize(records,plan,manifest_hash,smoke=False):
    ...
    return dict(schema='g1.pd.recorded-study.v1',complete=True,simulation_only=True,smoke=smoke,
        actual_runs=len(records),completed=sum(r['completed'] for r in records),eligible=sum(r['eligible'] for r in records),
        ranking=ranking,selected_simulation_vector=next((r['candidate'] for r in ranking if r['all_pass']),None) if not smoke else None,
        ...
        global_optimum_proven=False,scope='recorded outgoing7joint targets; fixed pelvis; both clocks; appended5s hold; not robot acceptance or measured response')
...
def audit_case(folder,job,c):
    ...
    proposed=np.clip(reference,previous-rates*.002,previous+rates*.002)
...
def audit(folder):
    ...
    require(m.get('simulation_only') is True and m.get('hardware_approved') is False and m.get('schema')=='g1.pd.recorded.manifest.v1','Manifest hardware/schema claim')
...
def main(argv=None):
    ...
    manifest=dict(schema='g1.pd.recorded.manifest.v1',simulation_only=True,smoke=args.smoke,
