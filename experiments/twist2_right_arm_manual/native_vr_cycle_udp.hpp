...
class NativeVrCycleUdp {
 int socket_=-1;
 in_addr source_{};
 std::string relay_token_;
 sockaddr_in peer_{};bool peer_seen_=false;
public:
 NativeVrCycleUdp(){
 ...
 NativeVrCycleUdp(const NativeVrCycleUdp&)=delete;
 NativeVrCycleUdp& operator=(const NativeVrCycleUdp&)=delete;
 ~NativeVrCycleUdp(){if(socket_>=0)::close(socket_);}
 void DiscardPending(){
  for(int n=0;n<4096;++n){
   char data[16385];
   const auto size=recvfrom(socket_,data,sizeof(data),0,nullptr,nullptr);
   if(size<0){if(errno==EAGAIN||errno==EWOULDBLOCK)return;throw std::runtime_error("VR discard failed");}
  }
  throw std::runtime_error("VR pre-sweep backlog overflow");
 }
 template<class Target> void Ack(Target& target){
  if(!peer_seen_)return;
  auto value=target.Acknowledgement(relay_token_).dump();
  if(sendto(socket_,value.data(),value.size(),0,reinterpret_cast<sockaddr*>(&peer_),sizeof(peer_))!=static_cast<ssize_t>(value.size()))throw std::runtime_error("cycle ack send");
 }
 template<class Target,class ClockFunction> void Drain(Target& target,ClockFunction now){
  for(int n=0;n<=64;++n){
   ...
   if(packet.at("schema")!="g1.mink.cycle.live.v1"||packet.at("command_provenance")!="live_mink"||packet.value("simulation_only",false)||packet.at("relay_token")!=relay_token_)throw std::runtime_error("cycle relay binding");
   peer_=sender;peer_seen_=true;
