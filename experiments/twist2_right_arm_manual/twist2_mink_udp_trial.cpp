...
int main(int argc, char** argv) {
  ...
  try {
    ...
    if (keyboard_right_arm && !pd_trial) {
      ...
      std::cout<<"[UDP] Absolute Mink targets: max 0.7 rad/s, 10 deg/s^2 (0.174533 rad/s^2); p stops. Wait for udp_ready before engage.\n";
    }
    std::cout
    ...
    if (keyboard_right_arm) {
      ...
      if(!pd_trial)std::cout<<"[UDP] Absolute Mink targets: max 0.7 rad/s, 10 deg/s^2 (0.174533 rad/s^2); p stops. Wait for udp_ready before engage.\n";
    }
    ...
    auto udp_target=std::make_shared<MinkUdpTarget>(baseline,clock(),clock,!pd_trial);
    std::unique_ptr<NativeVrUdp> udp;
    std::unique_ptr<PdReachTrial> trial;
    PdReadySettle pd_settle;
    double trial_started=-1;
    if(!pd_trial) {
      udp=std::make_unique<NativeVrUdp>();
      controller.set_input_watchdog([udp_target,clock](){return udp_target->Poll(clock());});
    }
      ...
      const LowState state = controller.snapshot(&state_age_ms,&received);
        ...
        if (keyboard_right_arm) {
          ...
          for(std::size_t i=12;i<29;++i){
            ...
            if(std::abs(state.motor_state()[i].dq())>kKeyboardMeasuredVelocityLimit)
              throw std::runtime_error("upper measured velocity exceeded 1.5 rad/s");
          }
          if(!pd_trial&&alpha>=1.0F)phase=udp_target->Ready()?"udp_ready":"arm_initializing";
        }
        std::array<float, kDofs> mimic_target{};
        for (std::size_t i = 0; i < kDofs; ++i) {
