...
int main(int argc, char** argv) {
  ...
  try {
    ...
    const bool keyboard_right_arm =
        argc == 7 && std::string(argv[3]) == "--enable-actuation" &&
        std::string(argv[4]) == "--policy-seconds" &&
        (std::string(argv[6]) == "--keyboard-right-arm" ||
         std::string(argv[6]) == "--keyboard-right-shoulder");
    ...
    if (keyboard_right_arm) {
      std::cout
          << "control: policy legs 0:12; captured waist/left arm held; "
             "right arm 7-DOF keyboard-commanded\n";
    } else {
      std::cout
    ...
    if (keyboard_right_arm) {
      std::cout
          << "keyboard timeline: after blend, right arm 7-DOF is "
      ...
      while (!controller.damping() && !gInterrupt.load()) {
        ...
        if (keyboard_right_arm) {
          ...
          for (const char raw_key : keyboard->read_available()) {
            ...
            std::size_t selected = kRightArmDofs;
            for (std::size_t arm = 0; arm < kRightArmDofs; ++arm) {
            ...
            const std::size_t joint = kRightArmBegin + selected;
            keyboard_command[joint] = update_keyboard_joint_command(
                ...
                kKeyboardIncrement);
            std::cout
                << "\n[keyboard] " << kRightArmNames[selected]
          ...
          if (keyboard_active) {
            ...
            for (std::size_t arm = 0; arm < kRightArmDofs; ++arm) {
              ...
              if (std::abs(
                ...
                throw std::runtime_error(
                    std::string("right arm tracking error exceeded 0.25 rad: ") +
                    kRightArmNames[arm]);
              }
              if (std::abs(state.motor_state()[joint].dq()) >
                  kKeyboardMeasuredVelocityLimit) {
                throw std::runtime_error(
                    std::string("right arm velocity exceeded 1.5 rad/s: ") +
                    kRightArmNames[arm]);
              }
            }
            phase = "keyboard";
