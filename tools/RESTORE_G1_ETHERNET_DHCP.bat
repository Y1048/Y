...
set "RC=%ERRORLEVEL%"
if not "%RC%"=="0" (
    echo [ERROR] Restoring Ethernet DHCP failed with code %RC%.
    echo [ACTION] Connect the ASIX AX88772A adapter, approve the administrator prompt, and retry.
    echo [ACTION] If it still fails, run RESTORE_G1_ETHERNET_DHCP_ADMIN.ps1 from an administrator PowerShell and inspect the detailed error.
)
endlocal & exit /b %RC%
